...
(defrecord HTML5History [hash-based? listeners generator current-uid prior-route all-events? prefix recent-history default-route
                         fulcro-app
                         route->url-fn url->route-fn]
                         
  RouteHistory
  (-push-route! [this route params]
    #?(:cljs
       (let [url (str prefix (route->url-fn route params hash-based?))]
  ...
  (-current-route [_] (url->route-fn hash-based? prefix)))
...
(defn new-html5-history
  "Create a new instance of a RouteHistory object that is properly configured against the browser's HTML5 History API.
   ...
   `route->url-fn` and `url->route-fn` - Overriding the default URL<->Route translation. Used to construct custom URL schemes.
  ...
  [{:keys [hash-based? all-events? prefix route->url-fn url->route-fn default-route app]
    :or {hash-based? false, all-events? false, prefix nil, route->url-fn route->url, url->route-fn url->route}}]
  ...
  #?(:cljs
     (try
       (let [history            (HTML5History. hash-based? (atom {}) (atom 1) (atom 1) (atom nil) all-events? prefix (atom []) default-route app route->url-fn url->route-fn)
